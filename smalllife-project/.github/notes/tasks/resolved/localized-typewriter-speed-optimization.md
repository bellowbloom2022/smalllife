...
## 2026-04-07 增量更新：LoadingPage Congrats 点击全出

### 需求

Level0/1/2/3 LoadingPage 中的 `level*_congrats` 文本也需要支持“打字中点击后立即全出”，统一全局文字播放体验。

### 实现

在 `LocalizedTypewriterEffect` 内新增空白点击跳过逻辑：

- `OnEnable()` 订阅 `InputRouter.OnBlankClickAnyButton`
- `OnDisable()` 取消订阅
- 回调中满足以下条件时执行 `SkipToEnd()`：
    - 当前文本正在打字（`IsTyping == true`）
    - `ShouldEnableBlankClickSkip()` 返回 true
...
- `enableBlankClickSkip = true`：当前组件强制开启点击跳过
- 否则若 `autoEnableBlankClickSkipForCongrats = true` 且 `phraseName` 含 `congrats`（忽略大小写），自动开启

### 生效对象（已确认）

- `Assets/Scenes/Level0LoadingPage.unity` → `phraseName: level0_congrats`
- `Assets/Scenes/Level1LoadingPage.unity` → `phraseName: level1_congrats`
- `Assets/Scenes/Level2LoadingPage.unity` → `phraseName: level2_congrats`
- `Assets/Scenes/Level3LoadingPage.unity` → `phraseName: level3_congrats`

### 回归与兼容

- 不影响 GoalDialog 既有首播/复播策略（由 `DialogueManager` 管理）
- 不改动 IntroChatController 的点击推进逻辑
- 非 `congrats` 文本默认行为不变
